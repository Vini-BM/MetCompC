...
function inicia()
    ...
    T=0.001
    Random.seed!(1234)
    x,y,vx,vy,X,Y=init_pos_vel(sqN,T)
    xv = Coord(x,y,vx,vy)
    N=length(xv.x)
...
function init_boxes(X,Y,N)
    ...
    lista=zeros(nf,ceil(Integer,4*N/nc))
    num=zeros(nc)
...
function forces_cells_intra(xv,dynpar,f,box)
    f.fx .= 0.0 ; f.fy .= 0.0
    #in box forces
    for c=1:box.nc
        n=box.num[c]  #particles in box c
        if box.nc > 1
            fpx=zeros(n,n)
            fpy=zeros(n,n)
...
function forces_cells_inter(xv,dynpar,f,box)
# Now we calculate the forces among particles of c and the neighboring boxes
     for c=1:box.nc-1
     	 n=box.num[c]  #particles in cell c
	 listv=[]
	 if box.nc > 0
	    listc=box.lista[c,1:n]
	    n1=box.num[box.v1[c]]
	    ...
	    nv=n1+n2+n3+n4
	    if nv > 0
	       if n1>0 push!(listv,box.lista[box.v1[c],1:n1])
	       end
	       if n2>0 push!(listv,box.lista[box.v1[c],1:n2])
	       end
	       if n3>0 push!(listv,box.lista[box.v1[c],1:n3])
	       end
	       if n4>0 push!(listv,box.lista[box.v1[c],1:n4])	   
	       end    
	    end
	end
...
function evol(xv,f,dynpar)
#    f=force(xv,dynpar,f)
    f=forces_cells_intra(xv,dynpar,f,box)
    f=forces_cells_inter(xv,dynpar,f,box)
    t=0;icount=1
    dt = dynpar.dtt
    while t < dynpar.tmax
        t +=dt;icount += 1
        xv,f=move(xv,dynpar,f)
